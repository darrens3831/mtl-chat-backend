...
// avec la clé secrète Stripe — jamais en faisant confiance au navigateur seul.
app.get('/verify-session', async (req, res) => {
  try {
    ...
    if (session.payment_status === 'paid') {
      ...
      const days = PLANS[plan]?.days || 0;
      return res.json({ paid: true, plan, days });
    }
    res.json({ paid: false });
  } catch (err) {
    console.error(err);
...
// ---------- (Optionnel mais recommandé) Webhook Stripe ----------
// Stripe peut aussi t'avertir directement ici dès qu'un paiement est confirmé,
// même si l'utilisateur ferme son navigateur avant de revenir sur le site.
// Pratique si tu ajoutes une vraie base de données plus tard.
